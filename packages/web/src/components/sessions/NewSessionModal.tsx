          ...
          >
            ...
            <form onSubmit={handleSubmit} className="px-5 pb-20 md:pb-5 space-y-5">
              ...
              {!checkingMachine && machinePaired === false && (
                <div className="rounded-2xl border border-warning/15 bg-warning/5 p-5 text-center space-y-3">
                  ...
                  <Link
                    to="/settings"
                    onClick={onClose}
                    className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-accent text-white text-xs font-semibold active:scale-[0.97] transition-all"
                  >
              ...
              <div className="space-y-2.5">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {providers?.map((p) => {
                    ...
                    const color = PROVIDER_COLORS[p.id] ?? "var(--color-accent)";
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "relative flex flex-col items-center gap-2 p-3.5 rounded-xl border transition-all duration-200",
                          isSelected
                            ? "border-white/[0.08] bg-surface-2/60"
                            : "border-white/[0.04] bg-surface-2/20 hover:bg-surface-2/40 hover:border-white/[0.06]",
                      ...
                      >
                        <div
                          className="h-10 w-10 rounded-xl flex items-center justify-center provider-icon"
                          style={{
                            background: `linear-gradient(135deg, ${color}18, ${color}08)`,
                            border: `1px solid ${color}${isSelected ? "30" : "15"}`,
                          }}
                        >
                          <span
              ...
              {selectedProviderConfig && (
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="w-full h-12 px-4 rounded-xl bg-surface-2/50 border border-white/[0.06] text-text-primary text-sm focus:outline-none focus:border-accent/30 transition-all appearance-none"
                  >
              ...
              {/* Session name */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-text-secondary">Session Name</label>
                <input
                  type="text"
                  value={sessionName}
                  onChange={(e) => setSessionName(e.target.value)}
                  placeholder={placeholder}
                  className="w-full h-12 px-4 rounded-xl bg-surface-2/50 border border-white/[0.06] text-text-primary text-sm placeholder:text-text-dimmed focus:outline-none focus:border-accent/30 transition-all"
                />
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-text-secondary">
                  Working Directory
                  <span className="text-text-dimmed ml-1 font-normal">(optional)</span>
                </label>
                <input
                  type="text"
                  value={workDir}
                  ...
                  className="w-full h-12 px-4 rounded-xl bg-surface-2/50 border border-white/[0.06] text-text-primary text-sm placeholder:text-text-dimmed focus:outline-none focus:border-accent/30 transition-all font-mono"
                />
              </div>
              ...
              <button
                ...
                className="w-full h-12 rounded-xl bg-accent text-white font-semibold text-sm transition-all disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center gap-2 active:scale-[0.97]"
              >
